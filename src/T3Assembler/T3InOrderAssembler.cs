...
namespace T3Assembler
{
    public class T3InOrderAssembler : T3AssemblerBase
    {
        readonly List<(
            int offset,
            string symbolName,
            T3ObjectFile.RelocationType type,
            T3ObjectFile.SectionType section
        )> _relocations = new();
        T3ObjectFile.SectionType _currentSection = T3ObjectFile.SectionType.TEXT;

        public T3InOrderAssembler(T3Config c)
            : base(c) { }
        ...
        public T3ObjectFile AssembleToObject(string src, string baseDir)
        {
            ...
            foreach (var l in raw)
            {
                ...
                {
                    var p = cl.Split(new[] { ' ', '\t' }, StringSplitOptions.RemoveEmptyEntries);
                    if (p.Length >= 2)
                        _globals.Add(p[1]);
                    continue;
                }
                ...
                if (ci != -1)
                {
                    ...
                    if (
                        !string.IsNullOrWhiteSpace(lb)
                        && lb.All(c => char.IsLetterOrDigit(c) || c == '_')
                    )
                    {
                        if (!_labels.ContainsKey(lb))
                            _labels[lb] = addr;
                        cl = cl[(ci + 1)..].Trim();
                    }
                }
                ...
                if (cl.StartsWith("equ ") || cl.Contains(" equ "))
                {
                    if (cl.Contains(" equ "))
                    {
                        var x = cl.Split(new[] { " equ " }, 2, StringSplitOptions.None);
                        if (x.Length == 2)
                            _constants[x[0].Trim()] = ResolveOperandValue(x[1].Trim());
                        continue;
                    }
                }
                if (cl.Contains(".string") || cl.Contains(".word"))
                {
                    var w = cl.Split(new[] { ' ', '\t' }, StringSplitOptions.RemoveEmptyEntries);
                    if (w.Length >= 2)
                    {
                        if (w[1] == ".string" || w[1] == ".word")
                        {
                            string lb = w[0];
            ...
            foreach (var g in _globals)
                if (_labels.TryGetValue(g, out int off))
                    obj.Symbols.Add(
                        new T3ObjectFile.Symbol
                        {
                            Name = g,
                            Type = T3ObjectFile.SymbolType.GLOBAL,
            ...
            foreach (var kv in _labels)
                if (!_globals.Contains(kv.Key) && !_externs.Contains(kv.Key))
                    obj.Symbols.Add(
                        ...
                        {
                            Name = kv.Key,
                            Type = T3ObjectFile.SymbolType.LOCAL,
                            Section = _currentSection,
                            Offset = kv.Value,
                        }
                    );
        ...
        int CalcSize(string line, int pc)
        {
            ...
            if (ip.Length == 0)
                return 0;
            string rawMn = ip[0];
            string mn = StripPrefix(rawMn).ToUpper();
        ...
        List<Int128> AssembleLine(string line, int pc)
        {
            ...
            if (line.Contains(".string"))
            {
                ...
                string c = line.Substring(fq + 1, lq - fq - 1);
                return ResolveString($"\"{c}\"");
            }
            if (line.StartsWith("\""))
            ...
            if (line.StartsWith(".align"))
            {
                var p = line.Split(new[] { ' ', '\t' }, StringSplitOptions.RemoveEmptyEntries);
                if (p.Length < 2)
                    throw new Exception(".align needs arg");
            ...
            if (line.StartsWith(".word"))
            {
                ...
                if (p[1].StartsWith("\""))
                    return ResolveString(p[1]);
                return new List<Int128> { ResolveOperandValue(p[1]) };
            }
            return AssembleCore(line, pc, null);
        }

        List<Int128> AssembleReloc(string line, int pc, T3ObjectFile obj) =>
            AssembleCore(line, pc, obj);
        ...
        List<Int128> AssembleCore(string line, int pc, T3ObjectFile? obj)
        {
            ...
            if (line.StartsWith("("))
            {
                ...
                if (cp != -1)
                {
                    ...
                    if (pp.StartsWith("p") && int.TryParse(pp[1..], out int pi))
                    {
                        pred = pi;
                        pl = line[(cp + 1)..].Trim();
                    }
                ...
                for (int k = 1; k < ip.Length; k++)
                {
                    if (IsRegister(ip[k]))
                    {
                        ...
                        if (regGroup != 0 && rg != 0 && rg != regGroup)
                            throw new Exception(
                                $"Architecture Error: Mixing registers from different groups in instruction {mn}. Operand {ip[k]} (Group {rg}) conflicts with detected Group {regGroup}."
                            );
                        if (rg == +1 && regGroup != +1)
                        {
                            regGroup = +1;
                        }
                        else if (rg == -1 && regGroup == 0)
                        {
                            regGroup = -1;
                        }
                    }
            ...
            {
                if (IsRegister(ip[1]))
                    op1 = GetRegisterRaw(ip[1]);
                else if (long.TryParse(ip[1], out long i1))
                    op1 = (int)i1;
            }
            if (ip.Length > 2)
            {
                if (IsRegister(ip[2]))
                    op2 = GetRegisterRaw(ip[2]);
                else if (long.TryParse(ip[2], out long i2))
                    op2 = (int)i2;
            }
            if (ip.Length > 3)
            {
                if (IsRegister(ip[3]))
                    op3 = GetRegisterRaw(ip[3]);
                else if (long.TryParse(ip[3], out long i3))
                    op3 = (int)i3;
            }
            ...
            if (ip.Length >= 2)
            {
                ...
                if (isImm)
                    imm = (long)ResolveOperandValue(last[1..]);
            }
            ...
            if (mn == "LIMM")
            {
                ...
                if (
                    obj != null
                    && (
                        !long.TryParse(valStr, out _)
                        && !valStr.StartsWith("0t")
                        && !valStr.StartsWith("0y")
                        && !valStr.StartsWith("0n")
                    )
                )
                {
                    if (
                        _externs.Contains(valStr)
                        || (!_labels.ContainsKey(valStr) && _globals.Contains(valStr))
                    )
                    {
                        int si = obj.Symbols.Count;
                        obj.Symbols.Add(
                            new T3ObjectFile.Symbol
                            {
                                Name = valStr,
                                Type = T3ObjectFile.SymbolType.EXTERN,
                                Section = T3ObjectFile.SectionType.ABS,
                                Offset = 0,
                            }
                        );
                        obj.Relocations.Add(
                            new T3ObjectFile.Relocation
                            {
                                SectionOffset = pc,
                                SymbolIndex = si,
                                Type = T3ObjectFile.RelocationType.LIMM_ABSOLUTE,
                                Section = _currentSection,
                            }
                        );
                        return new List<Int128>
                        {
                            EncI(pred, regGroup, Opcode.LIMM, op1, 0),
                            (Int128)0,
                        };
                    }
                }
                ...
                {
                    EncI(pred, regGroup, Opcode.LIMM, op1, 0),
                    (Int128)ResolveOperandValue(valStr),
                };
            }
            ...
            if (IsJumpMnemonic(mn))
            {
                ...
                if (!IsRegister(opn))
                {
                    long tgt = _labels.TryGetValue(opn, out int la)
                        ? la
                        : (long)ResolveOperandValue(opn);
                    if (
                        obj != null
                        && (
                            !_labels.ContainsKey(opn)
                            && (_externs.Contains(opn) || _globals.Contains(opn))
                        )
                    )
                    {
                        int si = obj.Symbols.Count;
            ...
            {
                string valStr = ip[1];
                Int128 v = ResolveOperandValue(valStr);
                if ((long)v > 364 || (long)v < -364)
                    return new List<Int128>
                    {
                        EncI(pred, regGroup, Opcode.LIMM, op1, 0),
                        (Int128)v,
            ...
            if (op == Opcode.LD || op == Opcode.ST)
            {
                ...
                {
                    string last = ip[ip.Length - 1];
                    if (!IsRegister(last) && !last.StartsWith("#"))
                        off = (long)ResolveOperandValue(last);
                }
            ...
            if (isImm || (ip.Length >= 3 && !IsRegister(ip[ip.Length - 1])))
                return new List<Int128>
                {
                    EncI(pred, regGroup, op, op1, (long)ResolveOperandValue(ip[ip.Length - 1])),
                };

            return new List<Int128> { EncR(pred, regGroup, op, op1, op2, op3) };
        }

        bool IsJumpMnemonic(string m) =>
            m is "JMP" or "JE" or "JNE" or "JL" or "JG" or "JM" or "JLE" or "JGE" or "CALL";

        static Int128 EncR(int pred, int rg, Opcode op, int o1, int o2, int o3) =>
            InstructionEncoder.EncodeR(pred, rg, 0, (int)op, o1, o2, o3);

        static Int128 EncI(int pred, int rg, Opcode op, int o1, long imm) =>
            InstructionEncoder.EncodeI(pred, rg, +1, (int)op, o1, imm);

        static Int128 EncJ(int pred, int rg, Opcode op, int reg) =>
            InstructionEncoder.EncodeJ(pred, rg, -1, (int)op, reg);

        static Int128 EncS(int pred, int rg, Opcode op, int o1, int o2, long imm) =>
            InstructionEncoder.EncodeS(pred, rg, 0, (int)op, o1, o2, imm);
    }
}
