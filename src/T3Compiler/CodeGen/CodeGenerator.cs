...
namespace T3Compiler.CodeGen
{
    public class CodeGenerator
    {
        ...
        readonly StringBuilder _codeOutput;
        int _labelCounter;
        readonly Dictionary<string, int> _varSlots = new(),
            _varSizes = new(),
            _varElemSizes = new();
        ...
        readonly Dictionary<string, List<FieldDef>> _structFields = new();
        readonly Stack<(string brk, string cont)> _loopStack = new();
        readonly Dictionary<string, List<FieldDef>> _structDefs = new();
        ...
        public string Generate()
        {
            ...
            foreach (var g in _program.Globals)
            {
                ...
                if (g.Type.Dims.Count > 0)
                    sz = g.Type.Dims.Aggregate(1, (a, b) => a * b);
            ...
            foreach (var g in _program.Globals)
            {
                ...
                if (g.Type.Dims.Count > 0)
                    sz = g.Type.Dims.Aggregate(1, (a, b) => a * b);
                Emit($"{g.Name}: .word {string.Join(", ", new long[sz])}");
            }
            Emit("\n; --- Data Section ---");
        ...
        void GenFunc(FunctionDef f)
        {
            ...
            void ScanBody(IEnumerable<Statement> body)
            {
                ...
                {
                    if (s is VarDeclaration vd)
                        Alloc(vd.Name, vd.Type);
                    else if (s is CompoundStmt cs)
                        ScanBody(cs.Body);
                    else if (s is IfStmt ifs)
                    {
                        if (ifs.ThenBody is CompoundStmt tcs)
                            ScanBody(tcs.Body);
                        else if (ifs.ThenBody is VarDeclaration tvd)
                            Alloc(tvd.Name, tvd.Type);
                        if (ifs.ElseBody is CompoundStmt ecs)
                            ScanBody(ecs.Body);
                        else if (ifs.ElseBody is VarDeclaration evd)
                            Alloc(evd.Name, evd.Type);
                        if (ifs.MaybeBody is CompoundStmt mcs)
                            ScanBody(mcs.Body);
                        else if (ifs.MaybeBody is VarDeclaration mvd)
                            Alloc(mvd.Name, mvd.Type);
                    }
                    else if (s is WhileStmt ws)
                    {
                        if (ws.Body is CompoundStmt wcs)
                            ScanBody(wcs.Body);
                        else if (ws.Body is VarDeclaration wvd)
                            Alloc(wvd.Name, wvd.Type);
                    }
                    else if (s is DoWhileStmt dws)
                    {
                        if (dws.Body is CompoundStmt dwcs)
                            ScanBody(dwcs.Body);
                        else if (dws.Body is VarDeclaration dwvd)
                            Alloc(dwvd.Name, dwvd.Type);
                    }
                    else if (s is ForStmt ft)
                    {
                        if (ft.Init is VarDeclaration fivd)
                            Alloc(fivd.Name, fivd.Type);
                        if (ft.Body is CompoundStmt fcs)
                            ScanBody(fcs.Body);
                        else if (ft.Body is VarDeclaration fvd)
                            Alloc(fvd.Name, fvd.Type);
                    }
                }
            }
            ...
            EmitCode("    S.PUSH FP");
            EmitCode("    POP RZ"); // RZ = FP (fixed frame base, PUSH-safe)
            int[] argRegs = { 0, 1, 2, 4 };
            ...
            if (nParams > 4)
            {
                for (int i = 4; i < nParams; i++)
                {
                    ...
                    else
                    {
                        int or = Imm(so),
                            ar = AllocR();
                        EmitCode($"    ADD {RegName(ar)}, RZ, {RegName(or)}");
                        EmitCode($"    LD {RegName(t)}, {RegName(ar)}, 0");
                        FreeR(or);
                        FreeR(ar);
                    }
                    StoreV(f.Parameters[i].Name, t, 0);
                    FreeR(t);
                }
            }
        ...
        void GenStmt(Statement s)
        {
            switch (s)
            {
                ...
                case VarDeclaration vd:
                    Alloc(vd.Name, vd.Type);
                    if (
                        vd.Type.StructName != null
                        && _structDefs.TryGetValue(vd.Type.StructName, out var sf)
                    )
                        _structFields[vd.Name] = sf;
                    if (vd.Initializer != null)
                    {
                        int r = GenExpr(vd.Initializer);
                        StoreV(vd.Name, r, 0);
                    }
                    break;
                case ReturnStmt rs:
                    if (rs.Value != null)
                    {
                        int r = GenExpr(rs.Value);
                        EmitCode($"    MOV R2,{RegName(r)}");
                    }
                    if (_epilogueLabel != null)
                    {
                        int rj = AllocR();
                        EmitCode($"    LIMM {RegName(rj)},{_epilogueLabel}");
                        EmitCode($"    JMP {RegName(rj)}");
                    }
                    else
                        EmitCode("    RET");
                    break;
                case CompoundStmt cs:
                    foreach (var ss in cs.Body)
                        GenStmt(ss);
                    break;
                case IfStmt ifs:
                    GenIf(ifs);
                ...
                case BreakStmt _:
                    {
                        var (brk, _) = _loopStack.Peek();
                        Jmp(brk);
                    }
                    break;
                case ContinueStmt _:
                    {
                        var (_, cont) = _loopStack.Peek();
                        Jmp(cont);
                    }
                    break;
                case GotoStmt gs:
                    Jmp($"__glbl_{gs.Label}");
                    break;
                case LabeledStmt ls:
                    EmitCode($"__glbl_{ls.Label}:");
                    GenStmt(ls.Body);
                    break;
            }
        }
        ...
        void GenFor(ForStmt fs)
        {
            ...
            if (fs.Condition != null)
                Jmp(lb); // This is a bit redundant but follows GenCondition logic
            EmitCode($"{lb}:");
            GenStmt(fs.Body);
            if (fs.Step != null)
                GenExpr(fs.Step);
        ...
        void JumpCond(string op, string l)
        {
            switch (op)
            {
                case "==":
        ...
        void JumpReg(string cond, string l)
        {
            int r = AllocR();
            EmitCode($"    LIMM {RegName(r)},{l}");
            EmitCode($"    {cond} {RegName(r)}");
        }
        ...
        void Jmp(string l)
        {
            int r = AllocR();
            EmitCode($"    LIMM {RegName(r)},{l}");
            EmitCode($"    JMP {RegName(r)}");
        }

        int GenExpr(AstNode n)
        {
            if (n is IntegerLiteral il)
                return Imm(ParseInt(il.Value));
            if (n is FloatLiteral fl)
                return EmitFloat(fl.Value);
            if (n is Identifier id)
            {
                if (_enumConstants.TryGetValue(id.Name, out int v))
                    return Imm(v);
            ...
            if (n is StringLiteral sl)
                return EmitString(sl.Value);
            if (n is BinaryOp bo)
                return GenBin(bo);
            if (n is UnaryOp uo)
                return GenUn(uo);
            if (n is Assignment ass)
                return EmitAssign(ass);
            if (n is ArrayAccess aa)
                return EmitArrAccess(aa);
            if (n is FunctionCall fc)
                return EmitCall(fc);
            if (n is MemberAccess ma)
                return EmitMemAccess(ma);
            if (n is TernaryExpr te)
                return GenTernary(te);
            throw new NotSupportedException($"Unsupported: {n?.GetType().Name}");
        }

        int GenTernary(TernaryExpr te)
        {
            int cr = GenExpr(te.Condition),
                r = AllocR();
            EmitCode($"    MOV R2,0");
            ...
            JumpReg("JG", lt);
            JumpReg("JE", lm);
            int fR = GenExpr(te.FalseExpr);
            EmitCode($"    MOV {RegName(r)},{RegName(fR)}");
            Jmp(ld);
            EmitCode($"{lm}:");
            int mR = GenExpr(te.MaybeExpr);
            EmitCode($"    MOV {RegName(r)},{RegName(mR)}");
            Jmp(ld);
            EmitCode($"{lt}:");
            int tR = GenExpr(te.TrueExpr);
            EmitCode($"    MOV {RegName(r)},{RegName(tR)}");
            EmitCode($"{ld}:");
            return r;
        }
        ...
        int EmitMemAccess(MemberAccess ma)
        {
            if (ma.Object is Identifier id && _structFields.TryGetValue(id.Name, out var fl))
            {
                ...
                if (off < 0)
                    throw new Exception($"Unknown field: {ma.MemberName}");
                int r = AllocR();
                if (_varSlots.TryGetValue(id.Name, out int ba))
                {
                    LabelAddr(ADDRREG, ba + off);
                    EmitCode($"    LD {RegName(r)},{RegName(ADDRREG)}, 0");
                }
                else if (_globalLabels.TryGetValue(id.Name, out string glbl))
                {
                    EmitCode($"    LIMM {RegName(ADDRREG)}, {glbl}");
                    EmitCode($"    LD {RegName(r)}, {RegName(ADDRREG)}, {off}");
                }
                else
            ...
            if (ma.Object is ArrayAccess aa && _structFields.TryGetValue(aa.ArrayName, out var fl2))
            {
                ...
                if (_varSlots.TryGetValue(aa.ArrayName, out int ba))
                    ...
                    throw new Exception($"Undefined array: {aa.ArrayName}");
                int tempR = AllocR();
                EmitCode($"    ADD {RegName(tempR)},{RegName(ADDRREG)},{RegName(scaledIdx)}");
                int offR = Imm(off);
                int finalAddrR = AllocR();
                EmitCode($"    ADD {RegName(finalAddrR)},{RegName(tempR)},{RegName(offR)}");
                int r2 = AllocR();
                EmitCode($"    LD {RegName(r2)},{RegName(finalAddrR)}, 0");
                FreeR(tempR);
                FreeR(offR);
                FreeR(finalAddrR);
                FreeR(scaledIdx);
            ...
            if (ma.Object is UnaryOp uo && uo.Operator == "*" && uo.Operand is Identifier ptrId)
            {
                foreach (var kv in _structDefs)
                {
                    ...
                    if (off >= 0)
                    {
                        ...
                        int offR = AllocR();
                        EmitCode($"    MOV {RegName(offR)},{off}");
                        int r = AllocR();
                        EmitCode($"    ADD {RegName(r)},{RegName(ptrR)},{RegName(offR)}");
                        EmitCode($"    LD {RegName(r)},{RegName(r)},0");
                        FreeR(offR);
                        return r;
                    }
                }
            ...
            throw new Exception($"Cannot access: {ma.MemberName}");
        }
        ...
        void EmitMemStore(MemberAccess ma, int v)
        {
            if (ma.Object is Identifier id && _structFields.TryGetValue(id.Name, out var fl))
            {
                ...
                if (off >= 0)
                {
                    ...
                    {
                        LabelAddr(ADDRREG, ba + off);
                        EmitCode($"    ST {RegName(v)}, {RegName(ADDRREG)}, 0");
                    }
                    else if (_globalLabels.TryGetValue(id.Name, out string glbl))
                    {
                        EmitCode($"    LIMM {RegName(ADDRREG)}, {glbl}");
                        EmitCode($"    ST {RegName(v)}, {RegName(ADDRREG)}, {off}");
                    }
                    else
                        throw new Exception($"Undefined variable: {id.Name}");
                }
            ...
            if (ma.Object is ArrayAccess aa && _structFields.TryGetValue(aa.ArrayName, out var fl2))
            {
                ...
                if (off >= 0)
                {
                    ...
                    EmitCode($"    ST {RegName(sv)}, {RegName(finalAddrR)}, 0");
                    FreeR(sv);
                    FreeR(tempR);
                    FreeR(offR);
                    FreeR(finalAddrR);
            ...
            if (ma.Object is UnaryOp uo && uo.Operator == "*" && uo.Operand is Identifier ptrId)
            {
                foreach (var kv in _structDefs)
                {
                    ...
                    if (off >= 0)
                    {
                        ...
                        EmitCode($"    ST {RegName(v)}, {RegName(addrR)}, 0");
                        FreeR(offR);
                        FreeR(addrR);
                        return;
                    }
        ...
        int GenUn(UnaryOp uo)
        {
            if (uo.Operator == "&")
            {
                ...
                {
                    if (_varSlots.TryGetValue(id.Name, out int a))
                        return ImmLabel(a);
                    if (_globalLabels.TryGetValue(id.Name, out string glbl))
                    {
                        int r = AllocR();
                        EmitCode($"    LIMM {RegName(r)},{glbl}");
                        return r;
                    }
                    throw new Exception($"Cannot take address of {id.Name}");
                }
                if (
                    uo.Operand is MemberAccess ma
                    && ma.Object is Identifier id2
                    && _structFields.TryGetValue(id2.Name, out var fl2)
                )
                {
                    int off = fl2.FindIndex(f => f.Name == ma.MemberName);
                    if (off < 0)
                        throw new Exception($"Unknown field: {ma.MemberName}");
                    if (_varSlots.TryGetValue(id2.Name, out int ba2))
                        return ImmLabel(ba2 + off);
                    else if (_globalLabels.TryGetValue(id2.Name, out string glbl))
                    {
                        int r = AllocR();
                        EmitCode($"    LIMM {RegName(r)}, {glbl}");
                        int offR = Imm(off);
                        int finalR = AllocR();
                        EmitCode($"    ADD {RegName(finalR)},{RegName(r)},{RegName(offR)}");
                        FreeR(r);
                        FreeR(offR);
                        return finalR;
                    }
                    else
                        throw new Exception($"Undefined variable: {id2.Name}");
                }
                if (uo.Operand is ArrayAccess aaa)
                {
                    int idx = FlatIdx(aaa);
                    int ra = AllocR();
                    if (_varSlots.TryGetValue(aaa.ArrayName, out int ba))
                        LabelAddr(ra, ba);
                    else if (_globalLabels.TryGetValue(aaa.ArrayName, out string glbl))
                        EmitCode($"    LIMM {RegName(ra)}, {glbl}");
                    else
                        throw new Exception($"Undefined array: {aaa.ArrayName}");
                    EmitCode($"    ADD {RegName(ra)},{RegName(ra)},{RegName(idx)}");
                    return ra;
                }
            ...
            if (uo.Operator == "*")
            {
                int pr = GenExpr(uo.Operand),
                    r = AllocR();
                EmitCode($"    LD {RegName(r)},{RegName(pr)},0");
                return r;
            }
            if (uo.Operator == "!")
            {
                int o = GenExpr(uo.Operand),
                    r2 = AllocR();
                EmitCode($"    CMP {RegName(o)},0");
                string lt = Lbl("t"),
                    ld = Lbl("d");
                JumpReg("JG", lt);
                EmitCode($"    MOV {RegName(r2)},1");
                Jmp(ld);
                EmitCode($"{lt}:");
                EmitCode($"    MOV {RegName(r2)},-1");
                EmitCode($"{ld}:");
        ...
        int GenBin(BinaryOp bo)
        {
            ...
            if (
                !IsCmp(bo.Operator)
                && bo.Left is IntegerLiteral il
                && bo.Right is IntegerLiteral ir
            )
            {
                long lv = ParseInt(il.Value),
                    rv = ParseInt(ir.Value);
            ...
            bool lf =
                bo.Left is FloatLiteral
                || (bo.Left is Identifier idL && _floatVars.Contains(idL.Name));
            bool rf =
                bo.Right is FloatLiteral
                || (bo.Right is Identifier idR && _floatVars.Contains(idR.Name));
            if (IsCmp(bo.Operator) && (lf || rf))
            {
                _fpuLive = true;
                int lr = GenExpr(bo.Left);
                EmitCode($"    F.LD {RegName(lr)},{RegName(lr)}");
                int rr = GenExpr(bo.Right);
                EmitCode($"    F.LD {RegName(rr)},{RegName(rr)}");
                int resReg = AllocR();
                EmitCode($"    F.CMP {RegName(lr)},{RegName(rr)}");
                FreeR(lr);
                FreeR(rr);
                string lt = Lbl("t"),
                    ld = Lbl("d");
            ...
            while (resultReg == l2 || resultReg == r1)
                resultReg = AllocR();
            string op = bo.Operator switch
            {
                ...
                "<<" => "SHL",
                ">>" => "SHR",
                _ => throw new NotSupportedException($"Unsupported: {bo.Operator}"),
            };
        ...
        int EmitAssign(Assignment ass)
        {
            ...
            if (ass.Target is Identifier id)
                StoreV(id.Name, v, 0);
            else if (ass.Target is ArrayAccess aa)
                EmitArrStore(aa, v);
        ...
        int EmitCall(FunctionCall fc)
        {
            ...
            if (fc.FunctionName == "malloc")
            {
                ...
                int retR = AllocR();
                EmitCode($"    S.PUSH HP");
                EmitCode($"    POP {RegName(retR)}");
                EmitCode($"    S.ADD HP, HP, {RegName(sizeR)}");
                return retR;
            }
            ...
            for (int i = 0; i < 4 && i < nArgs; i++)
            {
                ...
                for (int j = i + 1; j < 4 && j < nArgs; j++)
                {
                    ...
                    {
                        int t = AllocR();
                        while (t == argRegList[j] || t == argRegs[i])
                            t = AllocR();
            ...
            if (nArgs > 4)
            {
                ...
                EmitCode($"    S.ADD SP, SP, {stackSize}");
            }
            if (_fpuLive)
            {
                EmitCode("    POP R0");
        ...
        int FlatIdx(ArrayAccess aa)
        {
            ...
            for (int i = 0; i < dims.Count; i++)
            {
                ...
                int nextR = AllocR();
                while (nextR == r_restored || nextR == t)
                    nextR = AllocR();
        ...
        void EmitArrStore(ArrayAccess aa, int v)
        {
            ...
            int sv = AllocR();
            EmitCode($"    POP {RegName(sv)}");
            EmitCode($"    ST {RegName(sv)}, {RegName(ADDRREG)}, 0");
            FreeR(sv);
            FreeR(idxR);
        }
        ...
        void LabelAddr(int reg, int slotIndex)
        {
            ...
            if (offset < 0)
            {
                ...
                if (absOff <= 364)
                {
                    EmitCode($"    MOV {RegName(reg)},{absOff}");
            ...
            else
            {
                if (offset <= 364)
                {
                    EmitCode($"    MOV {RegName(reg)},{offset}");
        ...
        void Alloc(string name, TypeSpec ts)
        {
            if (!_varSlots.ContainsKey(name))
            {
                ...
                else if (ts.Dims.Count > 0)
                {
                    ...
                    if (
                        ts.StructName != null
                        && _structDefs.TryGetValue(ts.StructName, out var sf2)
                    )
                        elemSize = sf2.Count;
                    sz = count * elemSize;
        ...
        int LoadV(string name, int idx)
        {
            ...
            if (_varSlots.TryGetValue(name, out int a))
            {
                ...
                else
                {
                    ...
                    EmitCode($"    LD {RegName(r)}, {RegName(addrR)}, 0");
                    FreeR(offR);
                    FreeR(addrR);
                }
                return r;
            ...
            {
                EmitCode($"    LIMM {RegName(ADDRREG)}, {glbl}");
                EmitCode($"    LD {RegName(r)}, {RegName(ADDRREG)}, {idx}");
                return r;
            }
        ...
        void StoreV(string name, int reg, int idx)
        {
            ...
            if (_varSlots.TryGetValue(name, out int a))
            {
                ...
                else
                {
                    ...
                    int addrR = AllocR();
                    EmitCode($"    ADD {RegName(addrR)}, RZ, {RegName(offR)}");
                    EmitCode($"    ST {RegName(reg)}, {RegName(addrR)}, 0");
                    FreeR(offR);
                    FreeR(addrR);
                }
                return;
            ...
            {
                EmitCode($"    LIMM {RegName(ADDRREG)}, {glbl}");
                EmitCode($"    ST {RegName(reg)}, {RegName(ADDRREG)}, {idx}");
                return;
            }
            throw new Exception($"Undefined variable: {name}");
        }
        ...
        int EmitFloat(string value)
        {
            ...
            int r = AllocR();
            EmitCode($"    LIMM {RegName(r)},{lbl}");
            EmitCode($"    F.LD {RegName(r)},{RegName(r)}");
            return r;
        }
        ...
        int Imm(long v)
        {
            int r = AllocR();
            if (v >= -364 && v <= 364)
                EmitCode($"    MOV {RegName(r)},{v}");
            else
                EmitCode($"    LIMM {RegName(r)},{v}");
            return r;
        }
