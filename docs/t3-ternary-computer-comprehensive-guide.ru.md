...
### 1.1 Почему троичная логика?

Троичная логика использует три состояния: **−1**, **0**, **+1** (вместо двоичных 0 и 1). Это даёт несколько преимуществ:

| Свойство | Двоичная система | Троичная система |
|----------|------------------|------------------|
| Состояний на разряд | 2 | 3 |
| Информационная ёмкость | 1 бит | ~1.585 бит (log₂ 3) |
| Представление отрицательных чисел | Дополнительный код | Естественное (сбалансированная система) |
| Симметричность | Нет (−0 ≠ +0) | Да (знак — старший ненулевой трит) |
| Округление | К ближайшему чётному | Усечение (= отбрасывание) |

**Исторический контекст:** В 1958 году в МГУ под руководством Н.П. Брусенцова была построена ЭВМ **«Сетунь»** — первый и единственный серийный троичный компьютер (выпущено 50 экземпляров). T3 продолжает эту традицию, но как программная эмуляция с современным инструментарием.

---

## 2. Теория троичной логики

### 2.1 Трит — троичный разряд

**Трит** (ternary digit) — минимальная единица информации в троичной системе. Принимает три значения:

| Значение | Обозначение | Символ | Смысл |
|----------|------------|--------|-------|
...
### 2.2 Логические операции над тритами

В троичной логике стандартные двоичные операции обобщаются:

#### 2.2.1 Троичное И (AND) = Минимум
...
Результат — **минимальное** значение из двух операндов.
...
#### 2.2.2 Троичное ИЛИ (OR) = Максимум
...
#### 2.2.3 Троичное XOR = Сумма по модулю 3
...
```
TritXor(−1, +1) =  0
TritXor(+1, +1) = −1  (потому что 1+1 ≡ −1 mod 3)
TritXor( 0,  0) =  0
```

#### 2.2.4 Троичное НЕ (NOT) = Инверсия знака
...
### 2.3 Троичная логика Клейни

В некоторых контекстах используется трёхзначная логика Клейни (сильная логика неопределённости):

- **True** (+1): безусловная истина
- **False** (−1): безусловная ложь
- **Unknown/Maybe** (0): неопределённость

Эта логика встроена в язык T-lang через конструкцию `if`/`maybe`/`else`.

### 2.4 Тритовая арифметика

Арифметические операции над тритами выполняются **потритово** (как в двоичной системе, но с основанием 3):

- **Сложение:** `+ + + = +−` (1 + 1 = 2 в десятичной = +− в троичной, где `+` = 1, `−` = −1)
- **Умножение на 3:** сдвиг влево (`<<`)
- **Деление на 3:** сдвиг вправо (`>>`)
