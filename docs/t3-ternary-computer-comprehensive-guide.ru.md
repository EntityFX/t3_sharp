...
| Регистр | Индекс | Роль | Caller-saved? |
|---------|--------|------|---------------|
| **RW** | 0 (−4) | Аргумент 1 / Link-регистр (RET) | Да (не сохраняется) |
| **RX** | 1 (−3) | Аргумент 2 | Да |
| **RY** | 2 (−2) | Аргумент 3 / Возвращаемое значение | Да |
| **RZ** | 3 (−1) | Аргумент 4 / Frame Pointer | Да |
| **R0** | 4 (0) | Временный | Да |
| **R1** | 5 (+1) | Локальная переменная | **Нет** (callee-saved) |
| **R2** | 6 (+2) | Локальная переменная | **Нет** (callee-saved) |
| **R3** | 7 (+3) | Локальная переменная | **Нет** |
| **R4** | 8 (+4) | Локальная переменная | **Нет** |

Специальные регистры (отдельные поля):

| Регистр | Роль |
|---------|------|
| **SP** | Указатель стека (сохраняется при вызове) |
| **PC** | Счётчик команд |
| **Cond** | Флаг условия (−1/0/+1) |
| **PR** | Предикатный регистр |
| **ZR** | Нулевой регистр (всегда 0, индекс 8 в GP) |

### 8.2 Пролог функции

```
; Сохраняем callee-saved регистры если используются
PUSH R1
PUSH R2
; Выделяем место под локальные переменные
ADDI SP, SP, #-N    ; N = размер фрейма в трайтах
```
...
Старшие адреса ─────────────────────
                │ Аргументы (если > 4)      │
                │ Return address (в RW)     │ ← SP при входе
                │ Saved R1 (callee-saved)   │
                │ Saved R2 (callee-saved)   │
