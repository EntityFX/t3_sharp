...
*Примечание: текущий компилятор T-lang находится в стадии Beta. Он реализует базовый синтаксис, поддержку структур, массивов, функций, перечислений и строк. Хотя некоторые аспекты спецификации всё ещё в разработке, основные возможности языка полностью функциональны и протестированы.*
...
- **Десятичные с суффиксом размера:**  
  ...
  Примеры: `42y`, `-100t`, `100000tl`. (Поддерживается компилятором через суффиксы `y`, `s`, `t`, `tl`, `tll`).

- **Сбалансированные троичные:**  
  Префикс `0t`, затем последовательность `-`, `0`, `+` (длина до ширины типа). Допустимо `_`.  
  Примеры: `0t+-0`, `0t+++_+++` (макс. tryte), `0t---_---` (мин. tryte). (Поддерживается компилятором).

- **27‑ричные (tryx):**  
  Префикс `0y`, затем цифры из таблицы троек тритов. Каждая цифра кодирует 3 трита. (Поддерживается компилятором).
  Алфавит (троек от `---` до `+++`):  
  `N O P Q R S T U V` (--- … -++)  
...
- **9‑ричные (ninary):**  
  ...
  Примеры: `0nW04` (-- 00 ++), `0n000` (ноль tryte). (Поддерживается компилятором).
...
- Строки: `"текст"` — массив `tryte`. В текущей реализации компилятора строки хранятся в формате **«длина + данные»** (первое слово — количество символов, далее следуют сами символы), что позволяет эффективно определять длину строки.
...
Создают именованные целочисленные константы. В текущей реализации компилятора перечисления обрабатываются на этапе генерации кода как константы (immediate values), не занимая места в памяти как переменные.
