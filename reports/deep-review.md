...
## 10. T-lang compiler
 ...
 T-lang compiler существенно эволюционировал и перестал быть "самым слабым" компонентом. Он реализует полноценный pipeline: lexer $\rightarrow$ parser $\rightarrow$ AST $\rightarrow$ codegen. Тесты подтверждают поддержку сложной арифметики, вложенных циклов, многомерных массивов, структур и рекурсии.
 
 Ключевые улучшения затронули модель вызовов и ABI. Теперь реализована строгая модель сохранения регистров: caller-saved регистры (RW, RX, RY, RZ, R0, R1, R3, R4) сохраняются перед вызовом и восстанавливаются после. Параметры функций теперь передаются через стек и корректно извлекаются в прологе функции. Это решило проблему с порчей данных при глубоких вызовах и рекурсии.
 
 Языковые возможности расширены: добавлены `switch/case`, циклы `do/while`, тернарный оператор и составные операторы присваивания (+=, -= и т.д.). Также реализован полноценный доступ к полям структур, включая доступ через указатели и массивы структур.
 
 Остающиеся риски связаны с аллокатором регистров, который по-прежнему использует round-robin стратегию. Хотя новая модель ABI нивелировала многие проблемы с сохранностью данных, отсутствие анализа liveness может привести к неоптимальному использованию регистров в очень сложных выражениях. Также в коде были устранены "тихие" ошибки (silent fallbacks), теперь неподдерживаемые конструкции вызывают явные исключения `NotSupportedException`.
 
 Оценка T-lang compiler: **6.0 / 10**. Теперь это полноценный рабочий компилятор, который обеспечивает предсказуемое поведение и поддерживает широкий набор высокоуровневых конструкций.
