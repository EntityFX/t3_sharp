...
[TestMethod][Timeout(5000)]public void StackTest(){var p=new T3InOrderProcessor<Word18>(T3Config.T3_18);Ld(p,I(Opcode.LI,RW,100),R(Opcode.PUSH,RW,0,0),I(Opcode.LI,RW,200),R(Opcode.POP,RW,0,0),H());p.Run();Assert.AreEqual(100,p.Registers[0].ToLong());}
    ...
    [TestMethod][Timeout(5000)]public void PUSHI_POPI_Immediate(){var p=new T3InOrderProcessor<Word18>(T3Config.T3_18);p.SP=1000;Ld(p,I(Opcode.PUSHI,RW,42),I(Opcode.POPI,RW,0),H());p.Run();Assert.AreEqual(42,p.Registers[0].ToLong());}
    ...
    [TestMethod][Timeout(5000)]public void LI_max_immediate(){var p=new T3InOrderProcessor<Word18>(T3Config.T3_18);Ld(p,I(Opcode.LI,RW,364),H());p.Run();Assert.AreEqual(364,p.Registers[0].ToLong());}
    [TestMethod][Timeout(5000)]public void LI_min_immediate(){var p=new T3InOrderProcessor<Word18>(T3Config.T3_18);Ld(p,I(Opcode.LI,RW,-364),H());p.Run();Assert.AreEqual(-364,p.Registers[0].ToLong());}
    [TestMethod][Timeout(5000)]public void LI_out_of_range_throws(){try{I(Opcode.LI,RW,365);Assert.Fail("Should throw");}catch(ArgumentOutOfRangeException){}}
    [TestMethod][Timeout(5000)]public void ADDI_max_immediate(){var p=new T3InOrderProcessor<Word18>(T3Config.T3_18);Ld(p,I(Opcode.LI,RW,0),I(Opcode.ADDI,RW,364),H());p.Run();Assert.AreEqual(364,p.Registers[0].ToLong());}
}}
