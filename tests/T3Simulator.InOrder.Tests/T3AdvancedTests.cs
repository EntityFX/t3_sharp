...
[TestMethod,Timeout(3000)]public void Fibonacci(){var p=new T3InOrderProcessor<Word18>(T3Config.T3_18);Ld(p,I(Opcode.LI,RW,0),I(Opcode.LI,RX,1),I(Opcode.LI,RY,6),I(Opcode.LI,R0,5),R(Opcode.ADD,R3,RW,RX),R(Opcode.MOV,RW,RX,0),R(Opcode.MOV,RX,R3,0),I(Opcode.LI,R1,1),R(Opcode.ADD,RZ,RZ,R1),R(Opcode.CMP,RZ,RY,RZ),J(Opcode.JL,R0),H());p.Run();Assert.AreEqual(5,p.Registers[0].ToLong());}
