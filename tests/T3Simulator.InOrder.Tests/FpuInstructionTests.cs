...
[TestMethod][Timeout(5000)]public void FCLASS_Works(){var p=P();Ld(p,I(Opcode.LI,RW,5),R(Opcode.ITOF,FW,RW,0),R(Opcode.FCLASS,FW,FW,0),R(Opcode.FTOI,RW,FW,0),H());p.Run();Assert.AreEqual(4,p.Registers[0].ToLong());}
  [TestMethod][Timeout(5000)]public void FADD_Predicated_True(){var p=P();p.PR=Word18.FromLong(1);Ld(p,I(Opcode.LI,RW,10),R(Opcode.ITOF,FW,RW,0),I(Opcode.LI,RX,20),R(Opcode.ITOF,FX,RX,0),Word18.FromLong(InstructionEncoder.EncodeR(2,(int)Opcode.FADD,FZ,FW,FX)),R(Opcode.FTOI,RZ,FZ,0),H());p.Run();Assert.AreEqual(0,p.Registers[3].ToLong());}
  [TestMethod][Timeout(5000)]public void FADD_Predicated_Honored(){var p=P();p.PR=Word18.FromLong(1);Ld(p,I(Opcode.LI,RW,10),R(Opcode.ITOF,FW,RW,0),I(Opcode.LI,RX,20),R(Opcode.ITOF,FX,RX,0),Word18.FromLong(InstructionEncoder.EncodeR(1,(int)Opcode.FADD,FZ,FW,FX)),R(Opcode.FTOI,RZ,FZ,0),H());p.Run();Assert.AreEqual(30,p.Registers[3].ToLong());}
