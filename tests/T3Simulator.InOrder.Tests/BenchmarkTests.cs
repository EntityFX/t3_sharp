...
namespace T3Simulator.InOrder.Tests
{
    ...
    public class BenchmarkTests
    {
        ...
        [TestMethod, Timeout(3000)] public void Predication() { var p = new T3InOrderProcessor<Word18>(T3Config.T3_18); p.PR = Word18.FromLong(1); p.Registers[1] = Word18.FromLong(10); p.Registers[2] = Word18.FromLong(20); Ld(p, Word18.FromLong(InstructionEncoder.EncodeR(1, (int)Opcode.ADD, RW, RX, RY)), H()); p.Run(); Assert.AreEqual(30, p.Registers[0].ToLong()); }
        [TestMethod, Timeout(3000)] public void Quadratic() { var p = new T3InOrderProcessor<Word18>(T3Config.T3_18); Ld(p, I(Opcode.LI, RW, 1), I(Opcode.LI, RX, -3), I(Opcode.LI, RY, 2), R(Opcode.MUL, RZ, RX, RX), R(Opcode.MUL, R0, RW, RY), I(Opcode.LI, R1, 4), R(Opcode.MUL, R0, R0, R1), R(Opcode.SUB, RZ, RZ, R0), H()); p.Run(); Assert.AreEqual(1, p.Registers[3].ToLong()); }
        ...
        [TestMethod, Timeout(3000)] public void LoopSum() { var p = new T3InOrderProcessor<Word18>(T3Config.T3_18); Ld(p, I(Opcode.LI, RW, 0), I(Opcode.LI, RX, 1), I(Opcode.LI, RY, 6), I(Opcode.LI, R1, 1), I(Opcode.LI, R0, 4), R(Opcode.ADD, RW, RW, RX), R(Opcode.ADD, RX, RX, R1), R(Opcode.CMP, RX, RY, RZ), J(Opcode.JNE, R0), H()); p.Run(); Assert.AreEqual(15, p.Registers[0].ToLong()); }
