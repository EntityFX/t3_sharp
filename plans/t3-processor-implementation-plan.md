# T3 Ternary Processor — План реализации (исправленная версия)
...
| 1.3 | `Word18` struct | 18‑тритное слово для T3‑18, хранится в `int` (проверка границ) | `struct Word18` со сбалансированной троичной арифметикой, `ToLong()`, `FromLong()` |
| 1.4 | `Word54` struct | 54‑тритное слово для T3‑54, использует `BigInteger` или кастомное представление | `struct Word54`, арифметика, разложение на три 18‑тритных слота |
...
| # | Задача | Описание |
|---|--------|----------|
| 2.1 | `Opcode` enum | Базовые коды операций 0–27 (HALT…POP), свойство `PredicateIndex`; для I‑типов используется смещение +64 |
| 2.2 | `Instruction` struct | Декодированная инструкция: базовый опкод, предикат, op1, op2, op3, imm6, флаг I‑типа |
...
- Память отображает счётчики циклов по адресам `0x3FFFF00`–`0x3FFFF02` (18‑тритные адреса, соответствуют `0xFFFFFF00` в 27‑тритной нотации, но мы используем 18‑тритное пространство)
...
| 3.2 | Исполнительный движок | Центральный диспетчер для опкодов 0–27 (HALT…POP) |
...
| 3.6 | Операции с памятью | LOAD, STORE с пословной адресацией |
| 3.7 | Непосредственные операции | LI (imm6 из I‑типа), LIMM (следующее слово) |
...
| 4.9 | Полная поддержка опкодов | Все 45+ опкодов доступны на любом АЛУ |
...
| 5.12 | `InOrder.Tests` — Интеграционные | Факториал, рекурсивные вызовы (проверено: Фибоначчи), полноценные программы |
...
### Регистровое окно (при реализации в T3‑54)

```mermaid
stateDiagram-v2
    [*] --> Normal
    Normal --> CallExecuting: CALL инструкция
    CallExecuting --> WindowShifted: SP-=2, сохранить PC+WP, WP+=4
    WindowShifted --> Normal: RET инструкция
    WindowShifted --> Normal: Выполнение продолжается
    Normal --> StackFull: WP может переполниться
    StackFull --> Normal: Программное сохранение окна
```

---

### Проверенные интеграционные сценарии (In-Order)
- **Сложение массивов**: Проверена работа с линейной памятью, динамической индексацией и циклами.
- **Рекурсивный расчет чисел Фибоначчи**: Проверена корректность работы стека, передачи параметров и возврата значений при глубокой рекурсии (инструкции `CALL`, `RET`, `PUSH`, `POP`).
...
- `Word54` = 54 трита → требует `BigInteger` или собственную структуру из трёх `Word18`
...
Для инструкций I‑типа базовый опкод смещён на +64 относительно R‑типа (например, ADDI = ADD + 64). Альтернативно можно завести отдельные мнемоники.
